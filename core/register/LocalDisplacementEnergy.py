...
class BendingEnergy(LocalDisplacementEnergy):
    ...
    def forward(self, flow):
        ...
        if self.dimension == 2:
            ...
            return self.alpha * torch.mean(dfdxx**2 + dfdyy**2 + dfdzz**2 + 2 * dfdxy**2 + 2 * dfdxz**2 + 2 * dfdyz**2)
...
class JacobianDeterminant(LocalDisplacementEnergy):
    """
    Compute the Jacobian determinant at each spatial location for each subject
    the output is of shape [batch, num_subjects, *vol_shape]
